...
export function AiChatPage() {
  ...
  const switchConversation = async (conversationId: number) => {
    try {
      setIsLoading(true);
      const detail = await aiConversationService.getConversationDetail(
        conversationId
      );
      ...
      await sseService.connect(
        ...
        token,
        {
          ...
          onDone: (data: SSEChunk) => {
            ...
            console.log(
              "[AiChatPage] Current streaming message ID before complete:",
  ...
  const handleStreamComplete = () => {
    console.log(
      "[AiChatPage] handleStreamComplete called, currentStreamingMessageId:",
      currentStreamingMessageId.current
    );
      ...
      setMessages((prev) => {
        const updated = prev.map((msg) => {
          if (msg.tempId === messageIdToComplete) {
            console.log(
              "[AiChatPage] Setting isStreaming to false for message:",
              msg.tempId
            );
            return {
              ...msg,
        ...
        console.log(
          "[AiChatPage] Updated messages:",
          updated.map((m) => ({ tempId: m.tempId, isStreaming: m.isStreaming }))
        );
        return updated;
      });
    }, 0);
          ...
          <div className="flex items-end space-x-2">
            <div className="flex-1 flex items-end">
              <textarea
                ...
                className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent resize-none h-[50px] pb-[0px]"
                rows={3}
                disabled={isLoading}
              />
            </div>
