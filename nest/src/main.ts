...
async function bootstrap() {
  ...
  console.log(
    `   - GET ${protocol}://localhost:${port}/user?userId=test (用户信息)`
  );
  console.log(`   - GET ${protocol}://localhost:${port}/push (服务器推送演示)`);
  console.log("");
  console.log("🔐 认证端点:");
  console.log(
    `   - POST ${protocol}://localhost:${port}/auth/register (用户注册)`
  );
  console.log(
    `   - POST ${protocol}://localhost:${port}/auth/login (用户登录)`
  );
  console.log(
    `   - POST ${protocol}://localhost:${port}/auth/logout (退出登录)`
  );
  console.log(
    `   - GET ${protocol}://localhost:${port}/auth/profile (获取用户信息)`
  );
}
